    ...
    public class StrideTexture : TextureBase
    {
        ...
        public override void GenerateSolidColor(Thickness borderThickness, bool isBorder)
        {
            ...
            for (int i = 0; i < data.Length; i++)
            {
                ...
                {
                    data[i] = new Color(uint.MaxValue);
                }
                else
                {
            ...
            for (int y = 0; y < Height; y++)
            {
                for (int x = 0; x < Width; x++)
                {
                        ...
                        || !isBorder)
                    {
                        ...
                        startColor = startStop != null ? new Color(startStop.Color.PackedValue) : Color.Transparent;
                        GradientStop endStop = GetEndStop(finalOffset, sortedStops, spread);
                        endColor = endStop != null ? new Color(endStop.Color.PackedValue) : Color.Transparent;
                        ...
                        Color finalColor;                        
                        Color.Lerp(ref startColor, ref endColor, finalOffset, out finalColor);
